...
async def help(ctx):
    embed = discord.Embed(title="Hermes Tickets Help",
                          description="Use ,help <command> for more help on each specific command")
...
async def start(ctx):
    embed = discord.Embed(
        title="Start [Initiate]", description="Initializes the bot, creating all the necessary channels, roles, and db instances.")
...
async def promote(ctx):
    embed = discord.Embed(
        title="Promote", description="Gives mentioned member 'ticket-helper' role and allows them to view all tickets")

    embed.add_field(
        name="Usage", value=",promote <@user> | ,promote <user id>")
    await ctx.send(embed=embed)
...
async def demote(ctx):
    embed = discord.Embed(
        title="Demote", description="Removes 'ticket-helper' role from mentioned member and disallows them from viewing all tickets")
...
async def rename(ctx):
    embed = discord.Embed(
        title="Rename", description="Renames title of ticket channel")
...
async def end(ctx):
    embed = discord.Embed(
        title="End [Delete]", description="Deletes ticket channel upon confirmation")
